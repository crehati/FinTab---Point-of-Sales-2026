...
const BottomNavBar: React.FC<BottomNavBarProps> = ({ t, cart, currentUser, permissions }) => {
    ...
    const rawFinanceItems: TypedNavItem[] = [
        ...
        { to: '/bank-accounts', text: 'Bank Accounts', module: 'FINANCE', action: 'cash_count_enter' },
        { to: '/weekly-inventory-check', text: 'Weekly Inventory', module: 'FINANCE', action: 'weekly_inventory_check_enter' },
    ];
    ...
    return (
        <>
            {/* Extended Controls Drawer (Mobile Only) */}
            {isMenuOpen && (
                <div className="fixed inset-0 z-50 flex items-end justify-center p-4 sm:p-6 animate-fade-in font-sans">
                    ...
                    <div className="relative w-full max-w-md bg-white dark:bg-gray-900 rounded-[2.5rem] shadow-2xl overflow-hidden border border-white/10 animate-scale-in">
                        <header className="p-6 border-b dark:border-gray-800 flex justify-between items-center">
                            ...
                            <button onClick={() => setIsMenuOpen(false)} className="p-3 bg-slate-50 dark:bg-gray-800 rounded-full text-slate-400 focus:outline-none active:scale-90 transition-transform"><CloseIcon className="w-5 h-5" /></button>
                        </header>
                        ...
                        >
                            <div className="relative overflow-visible flex items-center justify-center scale-110">
                                ...
                                {item.badge !== undefined && item.badge > 0 && (
                                    <span className="absolute -top-1.5 -right-1.5 badge-standard bg-rose-500 min-w-[18px] h-[18px] !text-[8px] border-2 border-white dark:border-gray-900 flex items-center justify-center font-black animate-scale-in">
                                        {item.badge > 99 ? '99+' : item.badge}
                                    </span>
                                )}
                            </div>
                            <span className="text-[10px] font-bold tracking-tight whitespace-nowrap overflow-hidden text-ellipsis w-full text-center px-1 uppercase">
                                {item.text}
                            </span>
                        </NavLink>
                    ))}
